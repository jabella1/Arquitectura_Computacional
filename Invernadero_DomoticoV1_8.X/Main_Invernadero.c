...
void main(){
    ...
    ConfigurarPuertosLCD(); //Confugura el puerto D como salida para la pantalla lcd
    ...
    ApagarAspersores(); // Apaga los Vspersores
    ApagarVentiladores();//Apaga los Ventiladores
    ApagarLeds();        //Apaga los Leds
    ...
    while(1){   
     if(PrimeraVez==true){  
        ...
        ControlHumedadAspersor(&Datos[1],HumedadAdecuada,MargenEH); // se encarga de prender los aspersores
                                                 //en caso de que la humedad actual este por debajo de los
                                                 // limites.
        ControlTemperaturaVentiladorCalefactor(&Datos[0],TemperaturaAdecuada,MargenET); // se encarga de
